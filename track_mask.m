...
for ii=1:size(app.KleverImageLabellingToolKILTUIFigure.UserData.tracker,1)
	for kk = 1:size(app.KleverImageLabellingToolKILTUIFigure.UserData.tracker,2)
		if ~isempty(app.KleverImageLabellingToolKILTUIFigure.UserData.tracker{ii,kk})
			% Track the points. Note that some points may be lost.

			%use the points from previous frame
			oldPoints = app.KleverImageLabellingToolKILTUIFigure.UserData.tracker_points{ii,kk};

			if length(oldPoints) >= min_points_for_tracking
				[points, isFound] = step(app.KleverImageLabellingToolKILTUIFigure.UserData.tracker{ii,kk}, I_grayscale);
				...
				if numPoints >= min_points_for_tracking
					...
					my_polygon =  transformPointsForward(tform, squeeze(app.KleverImageLabellingToolKILTUIFigure.UserData.tracker_poly{ii,kk}));
